...
Vitals is a Windows terminal watchdog **and time-tracker** for 3ds Max (and other processes). It tracks CPU, RAM, VRAM, and Disk usage with a two-tier alert system, accumulates persistent weekly time stats (working / hanging / rendering / waiting), and orchestrates OS priorities under RAM pressure to protect an active render.
...
- **`vitals.py`** — Terminal UI, main monitor loop, alert state machine (`determine_state`), orchestration (`manage_orchestration`), VRAM/Storage daemons, `MemoryTracker` (deque sliding window), `_render_stats_box`, `print_report()`, `_is_work_hours()`, and the `vitals` console entry point.
...
7. **Tick classification** (priority order — also the order of the `>` indicator in the UI):
   - HANGING (any instance not responding)
   - rendering (CPU > threshold, or title contains "rendering")
   - away (waiting continuously for > `waiting_cutoff_seconds`; `record_tick` is skipped — no time recorded)
   - waiting (no Max foreground for `waiting_threshold_seconds`, OR outside work hours)
   - working
...
11. On process exit while last state was HANGING/CRITICAL → `record_exit` increments crash count.
...
| HANGING  | `IsHungAppWindow` returns true for the main window.           |
...
- `monitoring` — `refresh_interval_seconds`, `vram_monitor_interval_seconds`, `memory_tracker_window_size_seconds`, `waiting_threshold_seconds`, `waiting_cutoff_seconds` (seconds of continuous waiting before tracking pauses entirely; default 300).
- `schedule` — `enabled`, `work_start_weekday/hour`, `work_end_weekday/hour`. Outside this window, ticks count as waiting.
...
Keys are ISO Monday-of-week strings. Old weeks are **never** deleted — the UI surfaces the current week's bars and an **all-time** line aggregating every week. `SessionTracker._load` migrates legacy flat-format files transparently (including `idle_seconds` → `waiting_seconds`).

## Tests

Tests live in `tests/`. Windows-only ctypes are mocked so the suite runs on any platform.

Current suite: 135 tests, all passing.
