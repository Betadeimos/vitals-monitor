...
class TestSessionTracker(unittest.TestCase):
    ...
    def test_starts_empty(self):
        ...
        self.assertEqual(s["waiting_s"], 0.0)
    ...
    def test_record_tick_hanging(self):
        self.tracker.record_tick("HANGING", False, 1.0)
        s = self.tracker.summary()
        self.assertAlmostEqual(s["hanging_s"], 1.0)
        self.assertEqual(s["working_s"], 0.0)

    def test_hanging_takes_precedence_over_render(self):
        self.tracker.record_tick("HANGING", True, 1.0)
        s = self.tracker.summary()
        self.assertAlmostEqual(s["hanging_s"], 1.0)
        self.assertEqual(s["rendering_s"], 0.0)

    def test_hanging_takes_precedence_over_render_and_waiting(self):
        self.tracker.record_tick("HANGING", True, 2.0, is_waiting=True)
        s = self.tracker.summary()
    ...
    def test_waiting_swallows_normal(self):
        self.tracker.record_tick("NORMAL", False, 5.0, is_waiting=True)
        s = self.tracker.summary()
        self.assertAlmostEqual(s["waiting_s"], 5.0)
        self.assertEqual(s["working_s"], 0.0)

    def test_hanging_beats_waiting(self):
        self.tracker.record_tick("HANGING", False, 5.0, is_waiting=True)
        s = self.tracker.summary()
        self.assertAlmostEqual(s["hanging_s"], 5.0)
        self.assertEqual(s["waiting_s"], 0.0)

    def test_render_takes_precedence_over_waiting(self):
        self.tracker.record_tick("NORMAL", True, 3.0, is_waiting=True)
        s = self.tracker.summary()
        self.assertAlmostEqual(s["rendering_s"], 3.0)
        self.assertEqual(s["waiting_s"], 0.0)

    def test_billable_excludes_render_and_waiting(self):
        self.tracker.record_tick("NORMAL", False, 10.0)
        self.tracker.record_tick("HANGING", False, 5.0)
        self.tracker.record_tick("NORMAL", True, 3.0)
        self.tracker.record_tick("NORMAL", False, 8.0, is_waiting=True)
        s = self.tracker.summary()
        self.assertAlmostEqual(s["billable_s"], 15.0)
        self.assertAlmostEqual(s["active_s"], 18.0)
        self.assertAlmostEqual(s["total_s"], 26.0)

    def test_exit_hanging_counts_as_crash(self):
        self.tracker.record_exit("HANGING")
    ...
    def test_persistence(self):
        self.tracker.record_tick("HANGING", False, 60.0)
        self.tracker.record_tick("NORMAL", False, 10.0, is_waiting=True)
        self.tracker.record_exit("HANGING")
        self.tracker.save()
        t2 = SessionTracker(stats_file=self.path)
        s = t2.summary()
        self.assertAlmostEqual(s["hanging_s"], 60.0)
        self.assertAlmostEqual(s["waiting_s"], 10.0)
    ...
    def test_migrates_idle_seconds_to_waiting_seconds(self):
        old = {
            "weeks": {
                "2020-01-06": {
                    "working_seconds": 100.0,
        ...
        with open(self.path, "w") as f:
            json.dump(old, f)
        t = SessionTracker(stats_file=self.path)
        old_week = next((w for w in t.all_weeks() if w["week_start"] == "2020-01-06"), None)
        self.assertIsNotNone(old_week)
        self.assertAlmostEqual(old_week["waiting_s"], 500.0)
