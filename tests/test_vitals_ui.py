...
class TestVitalsUI(unittest.TestCase):
    ...
    def test_stacked_ram_bar_math(self, mock_virtual_memory):
        ...
        GREEN  = "\033[32m"
        RESET  = "\033[0m"
        self.assertIn(f"{WHITE}{'■' * 20}{RESET}", output)
        self.assertIn(f"{GREEN}{'■' * 5}{RESET}", output)
    ...
    def test_draw_stacked_cpu_bar(self, mock_cpu_percent):
        ...
        GREEN = "\033[32m"
        RESET = "\033[0m"
        self.assertIn(f"{WHITE}{'■' * 16}{RESET}", bar)
        self.assertIn(f"{GREEN}{'■' * 4}{RESET}", bar)
    ...
    def test_render_ui_status_matrix(self, mock_cpu_count):
        ...
        CYAN  = "\033[36m"
        RESET = "\033[0m"
        expected_matrix = f"{CYAN}[ PRIORITY: Normal       ] [ CORES: 4/8   ]{RESET}"
        self.assertIn(expected_matrix, output)
